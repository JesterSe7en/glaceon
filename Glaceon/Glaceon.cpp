...
void ErrorCallback(int error, const char *description) {
  GERROR("GLFW Error: Code: {} - {}", error, description);
}
...
void KeyboardCallback(GLFWwindow *window, int key,
...
static void ImGuiFrameRender(VulkanContext &context, ImDrawData *draw_data) {
  std::vector<vk::Semaphore> image_available_semaphores =
      context.GetVulkanSync().GetImageAvailableSemaphores();
  ...
  // the swap chain
  vk::Result res = device.acquireNextImageKHR(
      swap_chain, UINT64_MAX, image_available_semaphores[semaphore_index],
      VK_NULL_HANDLE, &context.current_frame_index_);

  vk::CommandBuffer command_buffer =
      context.GetVulkanCommandPool()
          .GetVkFrameCommandBuffers()[context.current_frame_index_];
  ...
  }
  {
    ...
    info.flags = vk::CommandBufferUsageFlags(
  ...
  }
  {
    ...
    render_pass_begin_info.renderPass =
        context.GetVulkanRenderPass().GetVkRenderPass();
    render_pass_begin_info.framebuffer =
        context.GetVulkanSwapChain()
            .GetSwapChainFrames()[context.current_frame_index_]
            .frame_buffer;
    render_pass_begin_info.renderArea.extent =
        context.GetVulkanSwapChain().GetSwapChainExtent();
...
static void ImGuiFramePresent(VulkanContext &context) {
  ...
  present_info_khr.sType = vk::StructureType::
  ePresentInfoKHR;  // VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;
  present_info_khr.waitSemaphoreCount = 1;
  present_info_khr.pWaitSemaphores =
      &render_complete_semaphores[context.semaphore_index_];
...
static void RecordDrawCommands(vk::CommandBuffer command_buffer, uint32_t image_index) {
  ...
  render_pass_info.framebuffer =
      context.GetVulkanSwapChain().GetSwapChainFrames()[image_index].frame_buffer;
...
static void GameFrameRender(VulkanContext &context) {
  ...
  std::vector<vk::CommandBuffer> frame_command_buffers =
  ...
  submit_info.pWaitSemaphores =
      &image_available_semaphores[context.semaphore_index_];
